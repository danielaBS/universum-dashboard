...
import styles from './optionsMultipleChecks.module.css';
import storybook from '@talentumlab/storybook-design-system';

const OptionsMultupleChecks = (props) => {
  const {
    options,
    fontSizeOptions,
    colorOptions,
  ...
  const filteredFilters = options.filter((data) => {
    return data.name.toString().toLowerCase().includes(inputSearch.toLowerCase());
  });
  ...
  let divOptions = filteredFilters.map((option, i) => (
    <label
      className={styles.result__options_value}
    ...
    >
      <input
        ...
        onChange={() => optionSelected({ id: option.id, name: option.name }, option.id)}
      />
      {option.name}
    </label>
  ));
  return (
    <div
      className={styles.result__options_check}
      tabIndex={idOptions}
      onClick={() => setFocusCheck(true)}
    >
      <div className={styles.search_input}>
        <div className={styles.search__searchButton}>
          <div>
            <storybook.Image src={iconSearch} />
          </div>
        </div>
        <input
          type="search"
...
export default OptionsMultupleChecks;

OptionsMultupleChecks.propTypes = {
...
OptionsMultupleChecks.defaultProps = {
